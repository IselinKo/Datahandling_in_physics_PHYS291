...
//
   xx[0]=0.0; yy[0]=1.0;
   for(i=1;i<n;i++){
     xx[i]=xx[i-1]+(0.5+gRandom->Rndm());
     yy[i]=(1.0+0.25*gRandom->Rndm())*cos(0.3*i);
   }
TGraph * grcos = new TGraph(n,xx,yy);
grcos->SetTitle("Randomly displaced points of cos(0.3*x)");
...
//
ffunc->SetParameter(0,0.9);   // Fit needs any NONZERO value !! TRY (0,0.0)
ffunc->SetParameter(1,0.25); 

TF1 * ofunc = (TF1*) ffunc->Clone("ofunc");
ofunc->SetLineColor(kOrange);
//
cout << "\n----------------------------------------\n";
