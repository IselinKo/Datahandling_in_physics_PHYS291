{//  Brace needed only when used as a script/macro
 ...
 // Run by     root three_fun.C    or .x three_fun.C in root
 // and use the Menu View,  item Editor on the Graphics Window
 ...
 fcos.Draw();                  
 fsin.Draw("SAME");
 fup.Draw("SAME");           // if interactive, enter line
 ...
 fcos.SetTitle("  ");         // otherwise a wrong title
