{
struct stud { char NAM[100]; char COURSE[100]; int POINTS=0;};
//
  stud group[4];
  cout << "Storage before data entry\n";
  cout << group[0].NAM << "  " <<  group[0].COURSE << endl;
  cout << group[1].NAM << "  " <<  group[1].COURSE << endl;
  cout << "sizeof(group[0]) " << sizeof(group[0]) << endl;
  cout << "sizeof(group[1]) " << sizeof(group[1]) << endl;
